...
const Popovers: FC<IPopoversProps> = ({
	...
	const childrenProps = (children as ReactElement)?.props as any;

	const ON_CLICK = () => {
		if (trigger === 'click') setPopoverOpen(!popoverOpen);
		if (typeof children !== 'string' && childrenProps?.onClick) childrenProps.onClick();
	};

	const ON_MOUSE_OVER = () => {
		if (trigger === 'hover') setPopoverOpen(true);
		if (typeof children !== 'string' && childrenProps?.onMouseOver) childrenProps.onMouseOver();
	};

	const ON_MOUSE_LEAVE = () => {
		if (trigger === 'hover') setTimeout(() => setPopoverOpen(false), delay);
		if (typeof children !== 'string' && childrenProps?.onMouseLeave)
	...
	const PROPS = {
		className: classNames(
			...
			typeof children !== 'string' && childrenProps?.className,
		),
		onClick: ON_CLICK,
